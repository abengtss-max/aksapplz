...
- [ ] **Apply environment lacks approval gate** — `cd-template.yaml` Apply job runs auto-apply with no `environment: { name: apply }` block enforcing required reviewers. Production deploys should pause for manual approval. Add `environment:` block referencing the `apply` GitHub environment (already created by bootstrap with empty reviewers list — need to wire it up).
- [ ] **Resource name length safety — broader audit needed**. Fixed Key Vault (24) + Grafana (23) this session, but other 24-char-limited resources (Storage Account, future additions) should follow the same length-safe pattern. Consider extracting a reusable `naming` module or `length_safe_name(prefix, max)` local helper.
...
- [ ] **CD state-lock race when multiple commits land quickly** — back-to-back pushes triggered overlapping runs that raced for the tfstate lock. Add `concurrency: { group: cd-${{ github.ref }}, cancel-in-progress: false }` to CD workflow so later runs queue instead of erroring.
