...
- [x] Managed Prometheus + Grafana + diagnostic settings  *(Prometheus **ingestion** path verified live 2026-07-30: `dcr-prometheus-*` + private `dce-prometheus-*` + `ampls-*`. **Container Insights LOGS path NOT working** — see G2 Container Insights item. **Prometheus QUERY API not privately reachable** — no managed private endpoint on the Azure Monitor workspace — see G2 Prometheus-query item.)*
...
- [ ] **Managed-Prometheus query API is not reachable on the private network (no managed private endpoint on the Azure Monitor workspace).** *High — verified live 2026-07-30 on `amon-aksapplz-prod-swc`.* The workspace has `publicNetworkAccess = Disabled`, but the only thing wired into the AMPLS is the auto-created **ingestion DCE** (`privatelink.monitor.azure.com` holds private A-records for `dce-prometheus-*` → `.ingest` 10.10.22.6 / `.metrics.ingest` 10.10.22.7 / `.handler.control` 10.10.22.5, and generic monitor endpoints). There is **no A-record for the workspace query FQDN** (`amon-…prometheus.monitor.azure.com`), so it resolves to the **public** Azure Front Door edge and any PromQL call returns `Forbidden: Access through public network is disabled`. Per [Azure Monitor Private Link docs](https://learn.microsoft.com/azure/azure-monitor/logs/private-link-security): *"the DCE needs to be added to the AMPLS to support **ingestion**… to support **queries**, a **managed private endpoint** needs to be created for the workspace and added to the AMPLS."* We provision the ingestion DCE→AMPLS but never the query managed private endpoint. **Metrics ingestion itself works** (source `/metrics` + `ama-metrics` remote-write verified); only ad-hoc query from inside the VNet is blocked — Grafana works via its own provisioned link. Needs: create the Azure Monitor workspace **managed private endpoint** and add it to `ampls-*` (or accept Grafana-only as the private query surface and document it). *(The stale "query endpoint is public by design" note in the validation runbook Step 2 has been corrected.)*
- [~] **Container Insights logs (ContainerLogV2) never ingest on the private workspace.** *High — verified live 2026-07-30 on `aks-aksapplz-prod-swc`. Fix committed to code; tracked in [issue #34](https://github.com/abengtss-max/aksapplz/issues/34) — awaiting apply + re-validation.* The `omsagent` addon is enabled and `ama-logs` pods run 3/3, but the **only DCR associated with the cluster was `dcr-prometheus-*`** — there was **no Container Insights logs DCR/DCRA**. With the Log Analytics workspace set to `publicNetworkAccessForIngestion/Query = Disabled` (AMPLS-only), the agent pulled an empty config (`mdsd: "No JSON file found ... DCR Json data:"`), so **zero `Heartbeat` and zero `ContainerLogV2`** landed — no centralized container logs. Managed Prometheus works because it has its own DCR + private DCE in the AMPLS; the equivalent logs chain was never built. **Fix (in `terraform/modules/region/monitoring.tf`, all 3 trees):** added `azurerm_monitor_data_collection_rule.container_insights` (`Microsoft-ContainerInsights-Group-Default` streams, `enableContainerLogV2 = true`) → the Log Analytics workspace, plus `azurerm_monitor_data_collection_rule_association.container_insights` to the cluster. Private ingestion is already covered — the workspace is an AMPLS scoped service (`azurerm_monitor_private_link_scoped_service.law`), and config access is served by the existing cluster DCE association (`prometheus_dce`), so **no separate logs DCE is required** for this deployment. Re-validate via `az monitor data-collection rule association list` (expect `dcra-ci-*`) and a `ContainerLogV2`/`Heartbeat` KQL query. *(App-side structured logging itself is fine — the pod emits JSON to stdout; only shipping was broken.)*
