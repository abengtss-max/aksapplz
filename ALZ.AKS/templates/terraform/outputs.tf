...
output "ingress_next_steps" {
  ...
  value = var.enable_app_gateway ? join("\n", compact([
    ...
    var.appgw_tls_key_vault_secret_id == "" ? (var.appgw_tls_mode == "self_signed" ? "  2. TLS: self-signed certificate generated in Key Vault (dev/test only) - HTTPS:443 active; HTTP:80 redirects to HTTPS. Supply appgw_tls_key_vault_secret_id with a customer certificate for production (required)." : (var.appgw_tls_mode == "disabled" ? "  2. TLS: disabled - the gateway serves HTTP:80 only. Set appgw_tls_key_vault_secret_id (or appgw_tls_mode=self_signed) to enable HTTPS:443." : "  2. TLS: mode 'keyvault' selected but no appgw_tls_key_vault_secret_id set - supply the unversioned Key Vault secret URL of your certificate to activate HTTPS:443.")) : "  2. TLS: HTTPS:443 listener active (customer Key Vault certificate); HTTP:80 redirects to HTTPS unless appgw_https_only = true.",
